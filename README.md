...
`cdd` is a real tool, and also where I practise maintaining an open source
project with an AI agent doing the work. A GitHub issue is the unit of work:
the agent audits it, fixes it behind a test, opens the pull request and cuts
the release. I file issues, approve new functionality and read the run
report. [docs/workflow.md](docs/workflow.md) describes every step and the
